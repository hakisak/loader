...
/**
 * Class to add completion mechanism to combo boxes. The class assumes that the
 * look and field uses a JTextField as the combo box editor. A check should be
 * done to ensure this is true before adding this class to a ComboBox.
 * 
 ...
 * PlainDocument pd = new PlainDocument(); _filter = new
 * ComboCompleterFilter(myCombo); pd.setDocumentFilter(_filter);
 * tf.setDocument(pd);
 * 
 * @author ncochran
 * 
 */
public class ComboAutoCompleteFilter extends AutoCompleteFilter {
   public ComboAutoCompleteFilter(JComboBox combo) {
      _combo = combo;
   }
    ...
    * will be returned).
    * 
    * The JTextField will have it's document set to a new PlainDocument and the
    * returned filter will be set to autocomplete the contents. Use the returned
    * filter to set options such as case-sensitivity.
    * 
    * @param combo
    */
   static public ComboAutoCompleteFilter addCompletionMechanism(JComboBox combo) {
      if (!(combo.getEditor().getEditorComponent() instanceof JTextField))
         return null;

      JTextField tf = (JTextField) combo.getEditor().getEditorComponent();
      PlainDocument pd = new PlainDocument();
      ComboAutoCompleteFilter filter = new ComboAutoCompleteFilter(combo);
