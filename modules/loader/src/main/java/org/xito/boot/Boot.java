...
public class Boot {
    ...
    private static ErrorPromptHandler errorPromptHandler;
    ...
    private static void setupSecurity() {
        ...
        if (noSecurityProp != null && noSecurityProp.equals("true")) {
            ...
            SwingUtilities.invokeAndWait(new Runnable() {
                public void run() {
                    try {
                        ...
                        ErrorPromptHandler.PromptResult result = showYesNoError(title, subtitle, msg, null);

                        if (result == ErrorPromptHandler.PromptResult.NO) {
    ...
    public static void initialize() {
        ...
        if (isInitialized()) {
            showError("Initialize Error", "Problem initializing Bootstrap", "Can't execute Boot.main. BootStrap already loaded.", null);
            return;
        }
    ...
    private static void setupLogging() {
        ...
        if (logConfigFile.exists()) {
            ...
            try {
                logger.info("Using logging.properties: " + logConfigFile.getAbsolutePath());
            ...
            logger.info("No logging.properties exists in bootDir using defaults");
        }
    }
    ...
    private static void processBootProperties() {
        ...
        try {
            ...
            //load boot properties from
            //the boot dir.
            File bootFile = new File(bootDir_str, "boot.properties");
            if(bootFile.exists()) {
                bootProps.load(new FileInputStream(bootFile));

                //Log all Boot Properties
                logProperties(bootProps, "Boot Properties", Level.INFO);
            }
            else {
                logger.warning("No boot.properties using defaults");
                bootProps.setProperty(APP_NAME, Resources.bundle.getString("app.name.default"));
                bootProps.setProperty(TRUST_BOOT_DIR, Boolean.toString(true));
            }
    ...
    private static void shutdownError(String error, Throwable exp) {
        ...
        showError(Resources.bundle.getString("boot.error.title"), Resources.bundle.getString("boot.error.subtitle"), error, exp);
        endSession(true);
    }
    ...
    private static void setupAppIcon() {
        ...
        try {
            URL iconURL = null;
            iconURL = new URL(Boot.getBootDir().toURI().toURL(), appIconStr);
            ...
            if (appIconStr.startsWith("http") || appIconStr.startsWith("file")) {
                ...
                appIcon = new ImageIcon(Boot.getCacheManager().getCachedFileForURL(iconURL).toURI().toURL());
            } else {
                appIcon = new ImageIcon(iconURL);
            }
        } catch (Exception exp) {
            showError(Resources.bundle.getString("boot.error.title"),
                    Resources.bundle.getString("boot.error.subtitle"),
                    Resources.bundle.getString("icon.url.error"), exp);
            appIcon = null;
        }
    ...
    public static ErrorPromptHandler.PromptResult showYesNoError(final String title, final String subtitle, final String msg, final Throwable exp) {
        return null;
    }
     ...
     */
    public static void showError(String title, String msg, Throwable exp) {
        showError(title, null, msg, exp);
    }
     ...
     * Show an Error in the BootStrap
     */
    public static void showError(final String title, final String subtitle, final String msg, final Throwable exp) {
    ...
    private static class OnlineTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                String title = Resources.bundle.getString("check.online.title");
                String subtitle = Resources.bundle.getString("check.online.subtitle");
