...
void showExpMessage(JNIEnv *env, jthrowable *exp) {
   //jclass jcls;
   //jmethodID jmid;
   //jstring jstr;
   //const char* message;

   jclass jcls = (*env)->GetObjectClass(env, *exp);
   jmethodID jmid = (*env)->GetMethodID(env, jcls , "getMessage" , "()Ljava/lang/String;" );
   jstring jstr = (jstring)(*env)->CallObjectMethod(env, *exp , jmid);
   const char* message = (*env)->GetStringUTFChars(env, jstr, FALSE);
   printf("%s", message);

   //MessageBox(NULL, message, "Netx Exception Occured", MB_ICONERROR + MB_OK);
}
...
int main(int argc, char *argv[]) {
   ...
   if (foundLibPath == FALSE) {
      ...
      mb_result = showError(FALSE, appName, mb_message);
      exit(-1);
   }
   ...
   if (result == JNI_ERR ) {
      mb_result = showError(FALSE, appName, "JNI Error invoking the JVM\n\n");
      exit(-1);
   }
   ...
   exp = (*env)->ExceptionOccurred(env);
   if (exp != NULL) {
      mb_result = showError(FALSE, appName, "Exception thrown from main method\n");
      exit(-1);
   }
   (*env)->ExceptionClear(env);
