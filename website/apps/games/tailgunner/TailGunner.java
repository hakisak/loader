/******************************************************************************
  ...
  1.2,  01/25/2001: Updated to JDK 1.1.8.
...
class TGPoint {
  ...
  public void transform(double ax, double ay, double az, double m,
...
class TGObject {
  ...
  public void render(TGPoint v, double mag, int xoffset, int yoffset) {
    ...
    for (i = 0; i < this.vertices.size(); i++) {
      ...
      point.transform(this.ax, this.ay, this.az, this.m,
    ...
    for (i = 0; i < this.faces.size(); i++) {
      ...
      // Find the angle of the face with respect to the viewpoint. Only add the
...
public class TailGunner extends Applet
  ...
  public void run() {
    ...
    while (Thread.currentThread() == loopThread) {
      ...
      // In playing or ending mode, update moving objects and advance blast
      // counter if active.
      ...
      // In game ending mode, advance ending counter and start demo when it
      // hits zero.
  ...
  public void updateStars() {
    ...
    for (i = 0; i < NUM_STARS; i++) {
      if((starsRadius[i] *= .95) < starsMinRadius[i])
        initStar(i);
      starsX[i] = (int) (scrnWidth / 2  
                + starsRadius[i] * Math.sin(starsAngle[i]));
      starsY[i] = (int) (scrnHeight / 2 
                + starsRadius[i] * Math.cos(starsAngle[i]));
    }
  }
  ...
  public void updateShips() {
    ...
    for (i = 0; i < NUM_SHIPS; i++) {
      ...
      // Otherwise, move the ship.
        ...
        // If too close, let it take a shot then remove and generate a new
        // ship.
  ...
  public void paint(Graphics g) {
    ...
    for (i = 0; i < TGObject.list.size(); i++) {
      ...
      for (j = 0; j < shape.polygons.size(); j++) {
        ...
        // If this is a ship, is it targeted and are we firing? (If ships
    ...
    if (targeted && gameState != END) {
      ...
      s = speed + "mps";
      offGraphics.drawString(s, mx - x - (smallFm.stringWidth(s)
        + smallFontWidth), my);
    }
    ...
    if (gameState == DEMO) {
      ...
      s = copyInfo;
      offGraphics.drawString(s, (d.width - fm.stringWidth(s)) / 2, 4 * fontHeight);
      s = copyLink;
      offGraphics.drawString(s, (d.width - fm.stringWidth(s)) / 2, 5 * fontHeight);
      ...
      if (targeted) {
        ...
        offGraphics.drawString(s, (d.width - smallFm.stringWidth(s)) / 2, 6 * fontHeight);
      }
